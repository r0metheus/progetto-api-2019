...
bool validRelExists(vertex_t **head, char *id_orig, char *id_dest,
		    char *id_rel);
...
bool vertexExists(vertex_t **head, char *id_ent)
{
	...
	for (current_vertex = *head; current_vertex != NULL;
	...
	if (current_vertex == NULL)
		...
		for (current_node = current_vertex->head_node;
			...
			if (current_node->valid &&
			    strcmp(current_node->id_dest, id) == 0 &&
			    strcmp(current_node->id_rel, id_rel) == 0)
				return current_node;
...
bool validRelExists(vertex_t **head, char *id_orig, char *id_dest, char *id_rel)
{
	...
	if (current_vertex == NULL || current_vertex->head_node == NULL)
		...
		for (current_node = current_vertex->head_node;
			...
			if (current_node->valid &&
			    strcmp(current_node->id_rel, id_rel) == 0 &&
			    strcmp(current_node->id_dest, id_dest) == 0)
				return true;
...
void addInChrono(chrono_t **chrono_head, char *id_rel)
{
	...
	if (inChronoExists(chrono_head, id_rel))
		...
		if (current_chrono == NULL ||
		    strcmp(current_chrono->id_rel, id_rel) > 0)
...
historyNode_t *returnValidHistory(vertex_t *vertex, char *id_rel)
{
	...
	for (current_history = vertex->head_history; current_history != NULL;
		...
		if (current_history->valid && current_history->num > 0 &&
		    strcmp(current_history->id_rel, id_rel) == 0)
	...
	if (current_history != NULL) {
		...
		if (update == 0)
			...
			if (vertex->head_history == NULL) {
				vertex->head_history =
					new_historyNode(id_rel, NULL, update);
	...
	if (*head == NULL || toDelete == NULL)
		...
		for (current_vertex = *head; current_vertex != NULL;
			...
			for (current_history = current_vertex->head_history;
			     ...
			     current_history = current_history->next)
				delrel(head, toDelete->id_ent,
				       current_vertex->id_ent,
				       current_history->id_rel);

		// Deletes vertex
		for (current_vertex = *head;
		     current_vertex != NULL &&
		     strcmp(current_vertex->id_ent, id_ent) != 0;
		     current_vertex = current_vertex->next)
...
void report(vertex_t **head, chrono_t **chrono_head)
{
	...
	if (*chrono_head == NULL) {
		...
		for (current_vertex = *head;
		     current_vertex != NULL && relation_c == 0;
		     current_vertex = current_vertex->next)
			for (current_history = current_vertex->head_history;
			     current_history != NULL;
			     current_history = current_history->next)
		...
		for (current_chrono = *chrono_head; current_chrono != NULL;
		     current_chrono = current_chrono->next) {
			for (current_vertex = *head; current_vertex != NULL;
			     current_vertex = current_vertex->next)
				for (current_history =
					     current_vertex->head_history;
				     current_history != NULL;
				     current_history = current_history->next)
					if (current_history->num > max &&
					    current_history->valid &&
					    strcmp(current_history->id_rel,
						   current_chrono->id_rel) == 0)
